 ...
 *
 * File: runtime_sampling_constant.cpp
 *
 */
...
public:
  ...
  Rtn_Sampling (double percentage)
    : generator_ (),
      skip_constant_ (size_t (100.0 / percentage) - 1),      
 ...
 */
class runtime_sampling_constant : public OASIS::Pin::Tool <runtime_sampling_constant>
{
public:
  /**
   * Constructor.
   */
  runtime_sampling_constant (void)
    : fout_ (outfile_.ValueString ()),
      image_instrument_ (fout_, percentage_.Value ()),
      start_time_ (std::chrono::system_clock::now ()),
      end_time_ ()
  {
    this->init_symbols ();
...
// $PIN_ROOT/pin -t $PINPP_ROOT/lib/simple_sampling_constant.dll -p <percentage> -f <output_filename> -- <program>
KNOB <double> runtime_sampling_constant::percentage_ (KNOB_MODE_WRITEONCE, "pintool", "p", "20.0", "sampling percentage");
KNOB <string> runtime_sampling_constant::outfile_ (KNOB_MODE_WRITEONCE, "pintool", "f", "simple_sampling_constant.out", "output file name");
...
DECLARE_PINTOOL (runtime_sampling_constant);
